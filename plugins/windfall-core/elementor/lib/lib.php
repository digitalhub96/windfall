...
function windfall_insert_elementor($atts){
...
add_shortcode('windfall_elementor_template','Elementor\windfall_insert_elementor');
	...
	class Controls_Helper_Output{
		...
		*/
		public static function get_posts( $post_type = '', $numberposts = ''){
			...
			if ( is_array( $gen_posts ) && !empty( $gen_posts ) ) {
			  ...
			  $all_posts = esc_html__( 'No contact forms found', 'windfall-core' );
			}
			return $all_posts;
		}
		...
		*/
		public static function get_include_icons(){
			...
			// Line icon
			$line_icons = WINDFALL_EM_SHORTCODE_BASE_PATH . '/lib/icons/linea.json';
			$line_icons = file_get_contents($line_icons);
			$line_icons = json_decode($line_icons)->icons;
			
			//Themify icon
			$themify_icons = WINDFALL_EM_SHORTCODE_BASE_PATH . '/lib/icons/themify.json';
